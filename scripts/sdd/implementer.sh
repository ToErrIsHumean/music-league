...
usage() {
  cat <<'EOF'
Usage:
  scripts/sdd/implementer.sh --task <TASK-NN> --spec <path> --cycle <n> [options]
...
Options:
  ...
  --cycle <n>                   Implementation cycle number.
  --cc                          Use the Claude Code backend via scripts/sdd/run-role.sh.
                                Enables
                                git HEAD guard and defaults to --effort medium.
                                Incompatible with --sandbox, --reasoning-effort,
                                --profile, --json, --ephemeral.
  --cline                       Use the Cline backend via scripts/sdd/run-role.sh.
                                Enables git HEAD guard. Incompatible with --sandbox,
                                --reasoning-effort, --profile, --json, --ephemeral,
                                and --effort.
  --review-feedback <path>      Optional reviewer verdict path from a contested cycle.
  --deterministic-feedback <path>
                                Optional deterministic gate output for a same-cycle retry.
  --escalation-brief <path>     Optional post-escalation brief path for a fresh retry.
...
resolve_existing_path() {
  ...
  usage_error "implementer.sh requires realpath or readlink -f to resolve paths"
}
...
resolve_path_maybe_missing() {
  ...
  usage_error "implementer.sh requires realpath or readlink -m to resolve paths"
}
...
DIFF_OUTPUT_PATH=""  # Derived from TASK_ID after validation
...
DETERMINISTIC_FEEDBACK_PATH=""
ESCALATION_BRIEF_PATH=""
ROLE_WORKDIR=""
...
while (($# > 0)); do
  case "$1" in
    ...
    --review-feedback)
      require_option_value "$1" "${2-}"
      REVIEW_FEEDBACK_PATH="$2"
      shift 2
      continue
      ;;
    --deterministic-feedback)
      require_option_value "$1" "${2-}"
      DETERMINISTIC_FEEDBACK_PATH="$2"
      shift 2
      continue
      ;;
    --escalation-brief)
      require_option_value "$1" "${2-}"
      ESCALATION_BRIEF_PATH="$2"
      shift 2
      continue
      ;;
    --workdir)
...
# Derive per-task last-diff path from TASK_ID
# Transformation: TASK-03 -> task-03 -> ${APP_SDD_DOCS_DIR_REL}/last-diff-task-03.md
TASK_ID_LOWER="$(echo "${TASK_ID}" | tr '[:upper:]' '[:lower:]')"
DIFF_OUTPUT_PATH="${APP_SDD_DOCS_DIR_REL}/last-diff-${TASK_ID_LOWER}.md"
...
if [[ -z "${ROLE_WORKDIR}" ]]; then
  ...
  ROLE_WORKDIR="$(resolve_existing_path "${ROLE_WORKDIR}")"
fi

if [[ ! -d "${ROLE_WORKDIR}" ]]; then
  usage_error "Role working directory not found: ${ROLE_WORKDIR}"
fi
...
supplementary_input_count=0
[[ -n "${REVIEW_FEEDBACK_PATH}" ]] && supplementary_input_count=$((supplementary_input_count + 1))
[[ -n "${DETERMINISTIC_FEEDBACK_PATH}" ]] && supplementary_input_count=$((supplementary_input_count + 1))
[[ -n "${ESCALATION_BRIEF_PATH}" ]] && supplementary_input_count=$((supplementary_input_count + 1))

if (( supplementary_input_count > 1 )); then
  usage_error "--review-feedback, --deterministic-feedback, and --escalation-brief are mutually exclusive"
fi

if [[ -n "${REVIEW_FEEDBACK_PATH}" ]]; then
  if [[ "${REVIEW_FEEDBACK_PATH}" != /* ]]; then
    REVIEW_FEEDBACK_PATH="${REPO_ROOT}/${REVIEW_FEEDBACK_PATH#./}"
  fi

  REVIEW_FEEDBACK_PATH="$(resolve_existing_path "${REVIEW_FEEDBACK_PATH}")"

  if [[ ! -f "${REVIEW_FEEDBACK_PATH}" ]]; then
    usage_error "Review feedback file not found: ${REVIEW_FEEDBACK_PATH}"
  fi
fi

if [[ -n "${DETERMINISTIC_FEEDBACK_PATH}" ]]; then
  if [[ "${DETERMINISTIC_FEEDBACK_PATH}" != /* ]]; then
    DETERMINISTIC_FEEDBACK_PATH="${REPO_ROOT}/${DETERMINISTIC_FEEDBACK_PATH#./}"
  fi

  DETERMINISTIC_FEEDBACK_PATH="$(resolve_existing_path "${DETERMINISTIC_FEEDBACK_PATH}")"

  if [[ ! -f "${DETERMINISTIC_FEEDBACK_PATH}" ]]; then
    usage_error "Deterministic feedback file not found: ${DETERMINISTIC_FEEDBACK_PATH}"
  fi
fi

if [[ -n "${ESCALATION_BRIEF_PATH}" ]]; then
  if [[ "${ESCALATION_BRIEF_PATH}" != /* ]]; then
    ESCALATION_BRIEF_PATH="${REPO_ROOT}/${ESCALATION_BRIEF_PATH#./}"
  fi

  ESCALATION_BRIEF_PATH="$(resolve_existing_path "${ESCALATION_BRIEF_PATH}")"

  if [[ ! -f "${ESCALATION_BRIEF_PATH}" ]]; then
    usage_error "Escalation brief file not found: ${ESCALATION_BRIEF_PATH}"
  fi
fi
...
if [[ "${CC}" == true ]]; then
  PASSTHROUGH_ARGS+=("--workdir" "${ROLE_WORKDIR}")
  PASSTHROUGH_ARGS+=("--cc")
...
RUN_ROLE_ARGS=(
  --prompt "${APP_SDD_DOCS_DIR_REL}/implementer.md"
  "${PASSTHROUGH_ARGS[@]}"
  --param "task_id=${TASK_ID}"
  --param "cycle=${CYCLE}"
  --param "guidance_path=${APP_PM_GUIDANCE_PATH_REL}"
  --param "diff_output_path=${DIFF_OUTPUT_PATH}"
)

if [[ -n "${SPEC_SLICE_PATH}" ]]; then
  RUN_ROLE_ARGS+=(
    --param "spec_universal_path=$(repo_relative_path "${SPEC_UNIVERSAL_PATH}")"
    --param "spec_task_slice_path=$(repo_relative_path "${SPEC_TASK_SLICE_PATH}")"
  )
else
  RUN_ROLE_ARGS+=(--param "spec_path=$(repo_relative_path "${SPEC_PATH}")")
fi

if [[ -n "${REVIEW_FEEDBACK_PATH}" ]]; then
  RUN_ROLE_ARGS+=(--param "review_feedback_path=${REVIEW_FEEDBACK_PATH}")
fi

if [[ -n "${DETERMINISTIC_FEEDBACK_PATH}" ]]; then
  RUN_ROLE_ARGS+=(--param "deterministic_feedback_path=${DETERMINISTIC_FEEDBACK_PATH}")
fi

if [[ -n "${ESCALATION_BRIEF_PATH}" ]]; then
  RUN_ROLE_ARGS+=(--param "escalation_brief_path=${ESCALATION_BRIEF_PATH}")
fi
...
if [[ ( "${CC}" == true || "${CLINE}" == true ) && "${DRY_RUN}" == false ]]; then
  PRE_HEAD="$(git -C "${ROLE_WORKDIR}" rev-parse HEAD)"
  ...
  POST_HEAD="$(git -C "${ROLE_WORKDIR}" rev-parse HEAD)"

  if [[ "${PRE_HEAD}" != "${POST_HEAD}" ]]; then
    git -C "${ROLE_WORKDIR}" reset --soft "${PRE_HEAD}"
    echo "WARN: Implementer committed - reset to ${PRE_HEAD}" >&2
  fi
